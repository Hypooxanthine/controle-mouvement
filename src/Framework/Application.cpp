...
void Application::setOptimizationData(const float * data) {
    ...
    ou
    PDController** PDcontrollers = m_biped->getPDControllers(); PDcontrollers[0]->setGains(data[0],data[1]);
...
void Application::getOptimizationData(float * data) {
    ...
    ou
    PDController** PDcontrollers = m_biped->getPDControllers(); data[0] = PDcontrollers[0]->getKpGain();
    pour le gain Kp du premier PD controller
    */
}
...
void Application::Step(Settings* settings) {
	...
	*/
	...
	//if (!settings->pause) {
    	...
    	// Controle du bipede
	    m_biped->update(timeStep, *settings);
